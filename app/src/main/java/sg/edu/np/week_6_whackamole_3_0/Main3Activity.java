...
public class Main3Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.v(TAG,"Got username intent from Mainactivity");
        String username=getUsername.getStringExtra("username");
        UserData userData = dbHandler.findUser(username);
        Log.v(TAG,"found username");

        RecyclerView theRecyclerView=findViewById(R.id.recyclerView);
        CustomScoreAdaptor theCAdapter=new CustomScoreAdaptor(userData);
        LinearLayoutManager theLManager=new LinearLayoutManager(this);
        theRecyclerView.setLayoutManager(theLManager);
        theRecyclerView.setAdapter(theCAdapter);
        theRecyclerView.setItemAnimator(new DefaultItemAnimator());



        buttonback=findViewById(R.id.recyclerBack);
